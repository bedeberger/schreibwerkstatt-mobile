 ...
 * Token vorliegt) plus `X-Client-Version`, `X-Client-Platform`/`X-Client-Device`
 * (Geräte-Label im Revision-Viewer, siehe [ClientInfo]) und einen
 * aussagekräftigen `User-Agent`. Bei 401 wird das Token verworfen — die UI fällt
 * dann (über `TokenStore.isPaired`) zurück aufs Pairing.
 *
 * Entkoppelt von `TokenStore` über zwei Lambdas ([token] lesen, [onUnauthorized]
 * bei 401) — so bleibt der Interceptor ohne Keystore/Context unit-testbar.
 */
class AuthInterceptor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        val builder = chain.request().newBuilder()
            ...
            .header("X-Client-Device", ClientInfo.deviceName)
