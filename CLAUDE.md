...
- **Auth-Modell.** Geräte-Token (`swd_…`) wird **einmalig** im Pairing ausgestellt und verschlüsselt im `TokenStore` abgelegt. Danach setzt `AuthInterceptor` auf **jedem** Request `Authorization: Bearer swd_…` + `X-Client-Version`. **401 → Token verwerfen → `isPaired=false` → Navigation zurück zum Pairing** (siehe `AppNav`). Token nie loggen, nie in DataStore/Klartext ablegen.

- **Pairing läuft über die Cookie-Session im WebView, NICHT über den Bearer-Interceptor.** `PairingScreen` öffnet die interaktive Google-OIDC-Session des Servers im WebView und ruft `POST me/device-tokens` per injiziertem `fetch` (Cookie-Auth). Das `plain_token` aus der Create-Response ist die **einzige** Stelle, an der das Token im Klartext erscheint.
...
|---|---|---|
| `POST me/device-tokens` | Pairing (WebView) | `lib/device-auth.js`, `db/device-tokens.js`, `routes/admin-devices.js` |
| `GET config` | `ConfigApi` | `routes/…` (`/config`) |
